...
class SaveConfig:
    ...
    def from_app() -> "SaveConfig":
        """
        Stores encrypted account files in a top-level 'data/' directory by default.
        You can override by setting app.config["SAVE_DIR"].
        """
        save_dir = current_app.config.get("SAVE_DIR")
        if save_dir:
            base_dir = Path(save_dir)
        else:
            # project_root/data  (project_root is parent of app/ folder)
...
def _default_payload() -> Dict[str, Any]:
    # Minimal payload: exists only to validate password via decryption
...
def login_or_create_account():
    """
    Strict behavior:
      - create=true  => MUST create new file; fail if exists
      - create=false => MUST exist; fail if missing; decrypt must succeed
    """
    ...
    if not path.exists():
        return (
            jsonify(
                ok=False, error="Account does not exist. Check 'Create new account'."
            ),
            404,
        )
